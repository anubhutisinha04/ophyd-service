...
class ScalerIOC(PVGroup):
    ...
    # .CONT (one-shot vs auto-count) as a bo record. bluesky detectors stage
    # the scaler by setting count mode to one-shot (0), so this must exist and
    # accept the write for `count`/`E_ramp` and other plans to stage the device.
    # Auto-count is not simulated (the counting loop is always one-shot); the
    # value is stored and reflected back so staging completes cleanly.
    CONT = pvproperty(
        value="OneShot",
        name=".CONT",
