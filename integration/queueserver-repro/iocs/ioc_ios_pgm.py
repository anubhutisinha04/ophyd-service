...
class PGMIOC(PVGroup):
    ...
    # Rests at "Ready" and returns to "Ready" when a fly completes. The IOS
    # profile's E_ramp plan (ramp_plan) subscribes to this signal and treats the
    # transition back to "Ready" as the fly-scan-done event, so that resting
    # value must be "Ready" (not "Idle") for the plan to finish.
    Scan_Sts = pvproperty(
        value="Ready",
        name="}}Sts:Scan-Sts",
        enum_strings=("Idle", "Scanning", "Ready"),
